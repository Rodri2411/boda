window.initSave = function () {
  ...
  function update() {
    ...
    // un toque más agresivo y “premium”
    const textY = (-300 * progress);      // sube más
    ...
    if (heroText) {
      heroText.style.transform = `translate3d(0, calc(-7vh + ${textY}px), 0)`;
  ...
  if (scrollIndicator) scrollIndicator.addEventListener("click", goCountdown);
  if (saveBtn) saveBtn.addEventListener("click", goCountdown);
};
